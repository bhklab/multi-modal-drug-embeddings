...
molMeta = pd.read_csv(dirs.RAWDATA / "HDD"/ "colData.csv", 
...
VIABILITY = pd.read_csv(dirs.PROCDATA / "NCI60.csv",index_col=0)
...
colData = pd.read_csv(dirs.RAWDATA / "HDD"/ "colData.csv")
...
for combo in all_combos:
    ...
    affinities = [compute.make_affinity(x,K=k) for x in [view.values for view in views]]
...
results.to_csv(dirs.RESULTS/ 'ablation.csv')
...
for k in range(1,100):
    ...
    affinities = [compute.make_affinity(x,K=k) for x in [GEOM.values, LINCS.values, JUMP.values, RESP.values]]
    ...
    for name, data in zip(
        ['GEOM','LINCS','JUMP','NCI60','SNF','IntegrAO'],
        [GEOM,LINCS,JUMP,RESP,FUSED,S_final]):
        
        if name not in ['SNF','IntegrAO']:
            aff = compute.make_affinity(data.values)
        else:
            aff=data
        fpr, tpr, thresh = metrics.roc_curve(X.values.flatten(), aff.flatten())
        auc = metrics.roc_auc_score(X.values.flatten(), aff.flatten())
    ...
    if k%5==0:
        plt.savefig(dirs.RESULTS / f"moa_roc_{k}.png")
    plt.close()



results = pd.DataFrame(results)
results.to_csv(dirs.RESULTS/ 'auc.csv')
