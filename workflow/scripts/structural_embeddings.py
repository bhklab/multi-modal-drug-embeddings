...
def main(config:Dict):
    ...
    outdir = dirs.RESULTS / "Structure"
    os.makedirs(outdir,exist_ok=True)
    
    
    fpRadius, useBondTypes, includeChirality, fpSize =\
          unpack_parameters(config['MORGAN_PARAMETERS'])

    alignment_epochs, mu, nbhd_size, seed,emb_dim =\
          unpack_parameters(config['INTEGRATION_PARAMETERS'])
    ...
    integrator = MultiViewIntegrator(
                views = [fingerprints, chemberta_embeddings, geom],
                view_names=['fingerprints','chemberta','geom'],
    ...
    final_adjacency = pd.DataFrame(S_final,index = all_samples,columns=all_samples)
